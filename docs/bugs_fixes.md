...
5. Importance of precise requirement interpretation
...
**Type:** Staff Management Issue

### Issue Description
Staff members who were already in the shift table were still appearing in the staff selection popup when trying to add new staff. This caused confusion and potential duplication of staff assignments.

1. **Staff Filtering Problem**
   - Staff members that were already added to the table still appeared in the popup
   - No distinction between available and already-assigned staff
   - Potentially allowing the same staff to be added multiple times

2. **Component Communication Issue**
   - ShiftTable wasn't correctly communicating which staff were already in the table
   - StaffSelector was filtering based on IDs rather than actual staff names

### Root Cause
The staff filtering logic had two critical issues:
1. ShiftTable was passing staff IDs to StaffSelector, but StaffSelector was expecting to match these against staff IDs from the context
2. The IDs being passed didn't match up with the staff names in the table rows
3. The actual staff names from the table rows weren't being used to filter available staff
...
// In StaffSelector:
const filteredStaff = activeStaff.filter(staff => !currentStaffIds.includes(staff.id));
```

### Solution
1. **Direct Name-Based Filtering**
   - Created a new `staffNamesInTable` variable that directly extracts staff names from table rows:
   ```jsx
...
2. **Updated StaffSelector Filtering Logic**
   - Modified StaffSelector to filter based on staff names instead of IDs:
   ```jsx
...
3. **Added Detailed Debugging**
   - Added comprehensive logging to track:
     - Staff in table rows
     - Available staff from context
     - Filtered staff being displayed in the popup

### Prevention Strategies
1. **State Visibility**
   - Use debugging logs to make state transformations visible
   - Ensure component props have clear meaning and purpose
   - Document the expected format of data passed between components

2. **Filtering Logic**
   - Establish clear patterns for filtering shared resources
   - Use consistent identifiers (names vs IDs) across the application
   - Test edge cases with partial and complete datasets

3. **Testing Protocols**
   - Test UI workflows with existing data
   - Verify that components correctly reflect application state
   - Add explicit validation to prevent duplicate entries
